...
import eu.su.mas.dedaleEtu.mas.agents.Agents.BDIAgent;
import eu.su.mas.dedaleEtu.mas.agents.Agents.PracticaAgent;
import eu.su.mas.dedaleEtu.mas.agents.Agents.TankerAgent;
...
//						
//						//2) Give the name of your agent, MUST be the same as the one given in the entities file.
//						agentName="TankerDummy";
//						
//						//3) If you want to give specific parameters to your agent, add them here
//
//						Object [] entityParametersAA={"My parameters"};
//						
//						//4) Give the class name of your agent to let the system instantiate it
//						ag=createNewDedaleAgent(c, agentName, DummyTankerAgent.class.getName(), entityParametersAA);
//						agentList.add(ag);
		 ...
		 ***************/
				...
				//4) Give the class name of your agent to let the system instantiate it
				ag=createNewDedaleAgent(c, agentName, PracticaAgent.class.getName(), entityParametersT);
				agentList.add(ag);
//				
...
//		                Assert.assertNotNull("This container does not exist",c);
		                try {
		                    ag = c.createNewAgent(agentName,BDIAgent.class.getName(),entityParameters);
		                    agentList.add(ag);
		                    System.out.println(agentName+" launched");
		                } catch (StaleProxyException e) {
